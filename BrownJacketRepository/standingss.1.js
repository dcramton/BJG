...
// Calculate Stableford average for each player
	for (let p =0; p < plength; p++) {
		...
		console.log("save[p] before conversion", save[p]);
		if (isNaN(save[p])) {save[p]=0;}
		console.log("save[p] after conversion", save[p]);
		if (save[p] == 0) {continue;}
		console.log("right after continue", save[p]);	
