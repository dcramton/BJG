...
const BJ_TABLE_ID = "bjpointstable"; 
...
function calculatePlayerScores(bjdata, playerCount) {
    if (!bjdata || !bjdata.games || !Array.isArray(bjdata.games)) {
        console.error('Invalid bjdata structure');
...
function showBjs(bjdata) {
    ...
    fetch('/static/playersbj.json')
        ...
        .then(data => {
            ...
            updateTable(BJ_TABLE_HEADER + tableRows);
        })
        .catch(error => {
...
function updateTable(tableContent) {
    const tableElement = document.getElementById("bjpointstable");
    if (!tableElement) {
    ...
    const headerHTML = `
        ...
        <tr>
            ...
            <th class="numeric" style="text-align:center">Brown Jacket Points</th>
        </tr>
    `;

    // Create the full table HTML
    tableElement.innerHTML = `${headerHTML}${tableContent}`;

    // Get all rows except the headers for sorting
    let rows = Array.from(tableElement.rows).slice(2);  // Skip the two header rows
    ...
    // Rebuild table with headers and sorted rows
    tableElement.innerHTML = headerHTML + rows.map(row => row.outerHTML).join('');
}
